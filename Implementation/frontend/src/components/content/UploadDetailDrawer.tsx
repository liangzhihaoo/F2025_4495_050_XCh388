...
export default function UploadDetailDrawer({
  item,
  onClose,
  onOpenGallery,
}: Props) {
  ...
  return (
    <>
      {open && (
        <div className="fixed inset-0 h-screen w-screen bg-black/20 z-40" onClick={onClose} />
      )}
      <div
        role="dialog"
        aria-hidden={!open}
        className={`fixed right-0 top-0 h-screen w-[420px] bg-white shadow-xl p-6 overflow-y-auto z-50 transition-transform duration-200 ${
            ...
            <div className="grid grid-cols-4 gap-2">
              {item.images.map((image, index) => (
                ...
                >
                  <img
                    src={image}
                    alt={`Image ${index + 1} of ${item.brand || "product"}`}
